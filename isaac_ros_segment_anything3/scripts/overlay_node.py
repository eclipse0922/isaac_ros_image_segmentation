...
class OverlayNode(Node):
    ...
    def __init__(self):
        ...
        # Subscribe to sam3/input_image (the exact frame SAM3 processed)
        # and sam3/raw_segmentation_mask — both published together by sam3_node,
        # so they always have matching timestamps. Tight slop avoids any mismatch.
        sam3_qos = QoSProfile(depth=5, reliability=ReliabilityPolicy.RELIABLE)
        image_sub = message_filters.Subscriber(
            self, Image, 'sam3/input_image', qos_profile=sam3_qos)
        mask_sub = message_filters.Subscriber(
            self, Image, 'sam3/raw_segmentation_mask', qos_profile=sam3_qos)

        self._sync = message_filters.ApproximateTimeSynchronizer(
            [image_sub, mask_sub], queue_size=5, slop=0.05)
